...
class Download_Remote_Image {
	 ...
	 *     @type int $user_id		 User ID added by LP
	 * }
	 */
	...
	public function download() {
		...
		/*$existing_attachment_id = $this->check_image_exist($this->url);
	...
	private function sideload() {
		...
		$temp_file = download_url( $this->url, 100 );
	...
	private function get_filename( $mime_type ) {
		...
		//listing-title_md5hash_1.jpg

		$md5_hash = $this->attachment_data['md5_hash'];
		$post_id = $this->attachment_data['post_id'];
		$listing_title = get_post_field('post_title', $post_id);
		$counter = 0;
		
		if ( metadata_exists( 'post', $post_id, 'lp_image_upload_counter' ) ) {
			$image_number = get_post_meta( $post_id, 'lp_image_upload_counter', true );
			if($image_number>0){
		...
		update_post_meta($post_id, 'lp_image_upload_counter', $counter);
		$filename = $listing_title."_".$md5_hash."_".$counter;
	...
	private function insert_attachment( $file_path, $mime_type ) {
		...
		$attachment_id = wp_insert_attachment( $attachment_data, $file_path, $this->attachment_data['post_id'] );
	...
	private function check_image_exist($image_url){
		global $wpdb;
